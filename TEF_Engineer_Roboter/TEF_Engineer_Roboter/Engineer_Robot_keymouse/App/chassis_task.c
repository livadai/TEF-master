...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
   ...
   if(chassis_move_rc_to_vector->chassis_RC->rc.SG>0){
				if(chassis_move_rc_to_vector->chassis_RC->key_mouse.keyboard.w){
					vx_channel =W_SPEED;
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key_mouse.keyboard.s){
					vx_channel =S_SPEED;
				}
				else {
					vx_channel = 0; 
				...
				if(chassis_move_rc_to_vector->chassis_RC->key_mouse.keyboard.a){
					vy_channel =A_SPEED;
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key_mouse.keyboard.d){
					vy_channel =D_SPEED;
				}
				else {
					vy_channel = 0; 
	 ...
	 else if(chassis_move_rc_to_vector->chassis_RC->rc.SG<0){
	     //死区限制，因为遥控器可能存在差异 摇杆在中间，其值不为0
    rc_deadband_limit(chassis_move_rc_to_vector->chassis_RC->rc.Ch2, vx_channel, CHASSIS_RC_DEADLINE);//!@#
    rc_deadband_limit(chassis_move_rc_to_vector->chassis_RC->rc.Ch4, vy_channel, CHASSIS_RC_DEADLINE);
	 }

    vx_set_channel = vx_channel*0.5; //* -CHASSIS_VX_RC_SEN;
    vy_set_channel = -vy_channel*0.5;//* -CHASSIS_VY_RC_SEN;
