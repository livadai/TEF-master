...
static void M15_Set(void)
{
    ...
    BSP_M15_SetPID(1, 0x02, 1, 0, 0, 0);
    BSP_M15_SavePID(1);
    BSP_M15_SetPositionLimit(1, 0x02, 25, -25);
}

//static void M15_control(gimbal_control_t *gimbal_rc)
//{

//	  int16_t ls_value = gimbal_rc->gimbal_rc_ctrl->rc.LS;
//	
//	  // 将 RD 的值映射到 M15_speed，并进行范围限制
//		M15_SPEED = (int16_t)(((float)ls_value/1342)*M15_MAX_SPEED); 
//	
//		// 限制 M15_speed 的范围
//		if (M15_SPEED > M15_MAX_SPEED) M15_SPEED =M15_MAX_SPEED;
//		if (M15_SPEED < -M15_MAX_SPEED) M15_SPEED = -M15_MAX_SPEED;
//}
...
static void gimbal_set_control(gimbal_control_t *gimbal_control_control)
{
    ...
    if (gimbal_control_control->gimbal_mode == GIMBAL_STOP_MODE)
    {
		...
		{
			if (gimbal_control_control->gimbal_rc_ctrl->rc.SG>0){
										//① PITCH end
										//if (gimbal_control_control->gimbal_rc_ctrl->rc.Ch3 <-150)
										if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.left_button_down)
										{
										...
										else if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.right_button_down)
										{
												gimbal_control_control->gimbal_l2006_motor.speed_set = -LR2006_SPEED;
												gimbal_control_control->gimbal_r2006_motor.speed_set = -LR2006_SPEED;
										}
										...
										{   
														...
														if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.f)
														{
														...
														else if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.g)
														{
																gimbal_control_control->gimbal_l2006_motor.speed_set = -1.5*LR2006_SPEED;
																gimbal_control_control->gimbal_r2006_motor.speed_set = 1.5*LR2006_SPEED;
														}
				...
				//								gimbal_control_control->gimbal_r2006_motor.speed_set = 0;
										...
										//② D end66
										if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.z)
										{
												gimbal_control_control->gimbal_d2006_motor.speed_set = D2006_SPEED;
										}
										else if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.x)
										{
												gimbal_control_control->gimbal_d2006_motor.speed_set = -D2006_SPEED;
										}
										else
										...
										//S end
										if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.e)
										{
												gimbal_control_control->gimbal_s2006_motor.speed_set = 0;
										}
										else if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.r)
										{
												gimbal_control_control->gimbal_s2006_motor.speed_set = -0;
										}
										else
										...
										//J1 J2 end
										if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.ctrl)
										{
												gimbal_control_control->gimbal_j2_motor.speed_set =2000;
												gimbal_control_control->gimbal_j1_motor.speed_set = -2000;
										}
										else if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.shift)
										{
												gimbal_control_control->gimbal_j2_motor.speed_set = -2000;
												gimbal_control_control->gimbal_j1_motor.speed_set = 2000;
										}
										...
										{
												gimbal_control_control->gimbal_j1_motor.speed_set = 0;
												if(gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.v)
												{
													gimbal_control_control->gimbal_j2_motor.speed_set =2000; 
												
												}
												else if (gimbal_control_control->gimbal_rc_ctrl->key_mouse.keyboard.c)
												{
													gimbal_control_control->gimbal_j2_motor.speed_set =-2000; 
      ...
      else if(gimbal_control_control->gimbal_rc_ctrl->rc.SG<0){ 			
												//① PITCH end
								if (gimbal_control_control->gimbal_rc_ctrl->rc.Ch3 <-150)
								{
										gimbal_control_control->gimbal_l2006_motor.speed_set = LR2006_SPEED;
